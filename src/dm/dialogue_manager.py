    ...
    def _handle_core(self, session_id: str, session: Dict[str, Any], text: str) -> str:
        ...
        if session["status"] == "CONFIRMING_CHECKOUT":
            if any(kw in text for kw in ["好", "對", "確定", "是", "ok", "要", "是的"]):
                return cart_manager.submit_order(session)
        ...
        if rtype == "checkout":
            if session["pending_frames"]:
                ...
                return clarify_message(first.get("itemtype", "unknown"), first.get("missing_slots", []), first)
            if not session["cart"]:
        ...
        while i < len(session["pending_frames"]):
            ...
            if frame.get("missing_slots"):
                if clarify_msg is None:
                    clarify_msg = clarify_message(frame.get("itemtype", "unknown"), frame["missing_slots"], frame)
                i += 1
                continue
    ...
    def _process_pending_frames(self, session_id: str, session: Dict[str, Any], text: str) -> str:
        ...
        if pending.get("_is_combo_sub_item") and rtype == "drink" and session.get("current_combo_frame"):
            session["current_combo_frame"]["swap_drink"] = {
                "drink": pending.get("drink"), "size": pending.get("size"), "temp": pending.get("temp")
            }
        newly_done = []
        ...
        if msg:
            return prefix + msg
        if newly_done:
            summary = "、".join([f"{i.get('quantity', 1)}份 {cart_manager.format_item(i)}" for i in newly_done])
            ...
            if combo:
                ...
                if not subs:
                    ...
                    for s in subs:
                        ...
                        s["missing_slots"] = recompute_missing_slots(s.get("itemtype", "unknown"), s)
                        parsed.append(s)
