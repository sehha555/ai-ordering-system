...
# 同音字 + 俗稱正規化（ASR 修正後的文字 → 菜單品名）
NORMALIZE_MAP = {
    "飯團": "飯糰",
    "大冰奶": "大杯冰純鮮奶茶",
    "小冰奶": "中杯冰純鮮奶茶",
    # 完整品名先列（dict 順序替換）：防「大冰紅茶」被短 key 吃成「大杯冰紅茶茶」
    "大冰紅茶": "大杯冰紅茶",
    "中冰紅茶": "中杯冰紅茶",
    "小冰紅茶": "中杯冰紅茶",
    "大冰紅": "大杯冰紅茶",
...
def _route(text: str, current_order_has_main: bool = False) -> Dict[str, Any]:
    ...
    if any(kw in t for kw in REMOVE_INDEX_KEYWORDS) or (
        ("第" in t) and ("項" in t or "個" in t) and ("刪" in t or "取消" in t)
    ):
    ...
    if "蛋餅飯糰" in t:
        return {
            "route_type": "riceball",
            "needs_clarify": False,
            "note": "exact_sku_guard:egg_pancake_riceball",
        }
    ...
    for flavor, aliases in FLAVOR_ALIASES.items():
        if aliases in t:
            return {
                "route_type": "riceball",
                "needs_clarify": False,
                "note": f"hit:flavor({flavor})",
            }
    if any(rice in t for rice in RICE_KEYWORDS):
        return {
            "route_type": "riceball",
            "needs_clarify": False,
            "note": "hit:rice_keyword_fallback",
        }
    ...
    if (
        ...
        and "蛋" in t
        and not any(
            carrier_word in t for carrier_word in ["吐司", "饅頭", "漢堡", "貝果", "蛋餅", "飯糰"]
        )
    ):
        return {
            "route_type": "carrier",
            "needs_clarify": False,
            "note": "hit:tuna_egg_needs_carrier",
        }
