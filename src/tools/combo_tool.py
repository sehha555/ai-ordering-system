...
def build_cart_item(
    ...
    """驗證套餐必填欄位 + 組裝購物車品項 dict（不含 item_id）。
    不做 rice 售完訊息注入（由 caller 處理）。
    """
    ...
    display = combo_name
    if customization:
        display += f"（{customization}）"
    return {"ok": True, "item": item, "display_name": display}
...
class ComboTool:
    ...
    def _load_menu_data(self):
        ...
        for name in self.all_item_names:
            # Full simplification
            s = name
            for t in self.remove_tokens:
                s = s.replace(t, "")
            for p in self.regex_removals:
                s = re.sub(p, "", s)
            s = s.strip()
            if s:
                self.simple_name_to_canonical[s] = name
            ...
            sp = sp.strip()
            if sp and sp != s:
                self.simple_name_to_canonical[sp] = name

        for alias, can in self.manual_aliases.items():
            if can in self.item_name_to_category:
                self.simple_name_to_canonical[alias] = can
        ...
        for item in all_items:
            if item.get("category") == "套餐":
                ...
                if m:
                    short = m.group(1)
                    self.combo_index[short] = {
                        "price": item["price"],
                        "desc": m.group(2).strip(),
                        "full_name": fn,
                        "default_drink_canonical": None,
                    }
        ...
        for short in self.combo_index:
            ...
            for sub in subs:
                ...
                if sub.get("itemtype") == "drink":
                    self.combo_index[short]["default_drink_canonical"] = can
                for sname, mcan in self.simple_name_to_canonical.items():
                    if mcan == can:
                        if sname not in self.sub_item_to_combo_names:
                            self.sub_item_to_combo_names[sname] = []
                        if short not in self.sub_item_to_combo_names[sname]:
                            self.sub_item_to_combo_names[sname].append(short)

    def parse_combo_utterance(self, text: str) -> Optional[Dict[str, Any]]:
        for short in self.combo_index:
            if short in text:
                return {"itemtype": "combo", "combo_name": short, "quantity": 1, "raw_text": text}

        found = [name for name in self.sub_item_to_combo_names if name in text]
        if not found:
            return None
        if len(found) == 1 and (
            found[0] in self.alias_cfg.get("allow_single_item_keywords", [])
            or found[0] in self.simple_name_to_canonical
        ):
            return None

        res = set(self.sub_item_to_combo_names.get(found[0], []))
        for i in range(1, len(found)):
            res.intersection_update(self.sub_item_to_combo_names.get(found[i], []))
        if len(res) == 1:
            return {"itemtype": "combo", "combo_name": res.pop(), "quantity": 1, "raw_text": text}
        return None
    ...
    def find_canonical_drink_name(self, drink: str, size: str) -> Optional[str]:
        ...
        for cand in [f"{drink}{suff}", drink]:
            if cand in self.item_name_to_category:
                return cand
    ...
    def choose_default_by_price(
        self, candidates: List[str], p_old: int
    ...
    def quote_combo_price(self, frame: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not short or short not in self.combo_index:
            return {"ok": False, "message": f"找不到套餐：{short}"}
        ...
        if swap:
            ...
            if not old_can:
                return {"ok": False, "message": f"{short} 沒有預設飲料，無法進行替換。"}
            try:
    ...
    def explode_combo_items(self, frame: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for p in parts:
            ...
            if best:
                ...
                pf = {}
                if cat == "飯糰":
                    pf = {"itemtype": "riceball", "flavor": best.replace("飯糰", "")}
                elif cat in ("吐司", "漢堡", "饅頭"):
                    pf = {"itemtype": "carrier", "carrier": cat, "flavor": best.replace(cat, "")}
                elif cat == "飲品":
                    sz, can = None, best
                    if "(中)" in can:
                        sz, can = "中杯", can.replace("(中)", "")
                    elif "(大)" in can:
                        sz, can = "大杯", can.replace("(大)", "")
                    pf = {"itemtype": "drink", "drink": can.strip(), "size": sz}
                elif cat == "蛋餅":
                    pf = {"itemtype": "egg_pancake", "flavor": best.replace("蛋餅", "")}
                elif cat == "點心":
                    nm, qty = best, 1
                    ...
                    if m:
                        pf = {
                            "itemtype": "jam_toast",
                            "jam_toast": m.group(1).strip(),
                            "size": m.group(2).strip().replace("片", ""),
                        }
                if pf:
                    pf["raw_text"] = best
                    res.append(pf)
