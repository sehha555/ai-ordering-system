...
def _prose_already_asks(prose: str, missing) -> bool:
    """LLM prose 已在追問全部缺槽（該 slot ≥2 個選項詞同現 + 問號）→ 後端不疊問。
    ...
    for slot in missing:
        ...
        if sum(1 for m in markers if m in prose) < 2:
            return False
    return True
    ...
    if "[ADD:" in full_text:
        ...
        if failed:
            failed_msgs = [
                r.get("message", "")
                for r in failed
                if r.get("message") and not _prose_already_asks(full_text, r.get("missing"))
            ]
            if failed_msgs:
