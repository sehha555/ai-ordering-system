...
def test_set_item_quantity(dm_session):
    ...
    dm.handle(sid, "")  # 初始化 session

    result = tr.add_snack(flavor="薯餅", quantity=3)
    ...
    item_id = session["cart"][0]["item_id"]

    # set_item_quantity: 3 → 1
    result = tr.set_item_quantity(item_id=item_id, quantity=1)
    assert result["ok"] is True
    assert session["cart"][0]["quantity"] == 1

    # set_item_quantity: qty=0 removes the item
    result = tr.set_item_quantity(item_id=item_id, quantity=0)
    assert result["ok"] is True
    assert len(session["cart"]) == 0
...
def test_set_item_quantity_not_found(dm_session):
    ...
    result = tr.set_item_quantity(item_id="nonexistent_99", quantity=2)
